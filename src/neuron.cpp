...
void Neuron::feed_forward(const Network::Layer& prev_layer)
{
    ...
    {
        sum += prev_layer[n].get_output_value() * prev_layer[n].m_output_weights[m_neuron_id].m_weight;
    }
...
void Neuron::update_input_weights(Network::Layer& prev_layer)
{
    for (size_t n = 0; n < prev_layer.size(); ++n)
    {
        ...
        double old_delta_weight = neuron.m_output_weights[m_neuron_id].m_deltaWeight;
